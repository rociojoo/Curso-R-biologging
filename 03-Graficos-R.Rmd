...
```{r, eval = FALSE}
download.file(url = "https://raw.githubusercontent.com/tommyclay/alba-wind-behaviour/master/Data_inputs/GPS_SouthGeorgia_2012.csv", destfile = paste0(path_data,"GPS_SouthGeorgia_2012.csv"))
```

```{r}
alba <- read.csv(paste0(path_data,"GPS_SouthGeorgia_2012.csv"), stringsAsFactors = TRUE)
```
...
que consideran color blindness.
...
[20 minutos, levantar la mano digital al terminar]
...
sum_penguins <- penguins %>%
  ...
  summarise(mass = mean(body_mass_g, na.rm = TRUE), flipper = mean(flipper_length_mm, na.rm = TRUE))
...
proyeccion <- "+init=epsg:4326"
  # "+proj=aeqd +lat_0=-54.01 +lon_0=-38.05 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0" # si no hay más información de lon y lat 
...
# explicaré cómo bajar el archivo en clase o lo pasaré
...
plot_anim <- ggplot(data = alba_sftrack_sub, aes(color = ID)) +
  ...
  theme_light() +
  theme(legend.position = "none") +
  transition_states(tiempo)  + # en base a qué variable se hará la animación
...
Tiempo para explorar y graficar. [1 hora]
